import numpy as np
from old_rapprentice import conversions as conv, colorize, math_utils as mu, \
    retiming, PR2, resampling
from old_rapprentice import LOG
import IPython
...
def return_timed_traj(pr2, bodypart2traj, wait=True, base_frame = "/base_footprint", speed_factor=1):
...
def follow_body_traj(pr2, bodypart2traj, wait=True, base_frame = "/base_footprint", speed_factor=1):    
    ...
    IPython.embed()
    times = retiming.retime_with_vel_limits(trajectories, vel_limits)
    #times_up = np.linspace(0, trajectories.shape[0]*0.01,trajectories.shape[0])
    times_up = np.linspace(0, times[-1], int(np.ceil(times[-1]/.1)))
    traj_up = mu.interp2d(times_up, times, trajectories)
